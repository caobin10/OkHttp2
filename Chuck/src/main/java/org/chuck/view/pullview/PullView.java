 ...
 */
package org.chuck.view.pullview;
...
public class PullView extends LinearLayout {
	...
	/** Footer加载更多监听器. */
	private OnFooterPullListener mOnFooterLoadListener;
	
	/** Header下拉刷新监听器. */
	private OnHeaderPullListener mOnHeaderRefreshListener;
	...
	private void addHeaderView() {
        mHeaderView = new PullViewHeader(mContext);
	...
	private void addFooterView() {
        ...
        mFooterView = new PullViewFooter(mContext);
	...
	private void headerPrepareToRefresh(int deltaY) {
		...
		if (newTopMargin >= 0 && mHeaderView.getState() != PullViewHeader.STATE_REFRESHING) {
		    //提示松开刷新
		    mHeaderView.setState(PullViewHeader.STATE_READY);
		
		} else if (newTopMargin < 0 && newTopMargin > -mHeaderViewHeight) {
		    //提示下拉刷新
		    mHeaderView.setState(PullViewHeader.STATE_NORMAL);
		}
	}
	...
	private void footerPrepareToRefresh(int deltaY) {
		...
		if (Math.abs(newTopMargin) >= (mHeaderViewHeight + mFooterViewHeight) && mFooterView.getState()  != PullViewFooter.STATE_LOADING) {
			mFooterView.setState(PullViewFooter.STATE_READY);
		} else if (Math.abs(newTopMargin) < (mHeaderViewHeight + mFooterViewHeight)) {
		    mFooterView.setState(PullViewFooter.STATE_LOADING);
		}
	}
	...
	public void headerRefreshing() {
		...
		mHeaderView.setState(PullViewHeader.STATE_REFRESHING);
		setHeaderTopMargin(0);
		if (mOnHeaderRefreshListener != null) {
			mOnHeaderRefreshListener.onHeaderPull(this);
		}
	}
	...
	private void footerLoading() {
		...
		if (mOnFooterLoadListener != null) {
			mOnFooterLoadListener.onFooterPull(this);
		}
	}
	...
	public void onHeaderRefreshFinish() {
		...
		if(mAdapterView!=null){
			...
			if(mCount > 0){
				...
				mFooterView.setState(PullViewFooter.STATE_EMPTY);
			}
		}else{
			mFooterView.setState(PullViewFooter.STATE_READY);
		}
	...
	public void onFooterLoadFinish() {
		...
		if(mAdapterView!=null){
			...
			if(countNew > mCount){
				...
				mFooterView.setState(PullViewFooter.STATE_NO);
			}
		}else{
			mFooterView.setState(PullViewFooter.STATE_READY);
		}
	 ...
	 */
	public void setOnHeaderPullListener(OnHeaderPullListener headerRefreshListener) {
	 ...
	 */
	public void setOnFooterPullListener(OnFooterPullListener footerLoadListener) {
     ...
     */
	public interface OnFooterPullListener {
		 ...
		 * @param view the view
		 */
		public void onFooterPull(PullView view);
	}
	...
	public interface OnHeaderPullListener {
		 ...
		 * @param view the view
		 */
		public void onHeaderPull(PullView view);
	}
